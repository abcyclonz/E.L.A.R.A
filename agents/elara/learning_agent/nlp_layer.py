...
_CONFUSION_PATTERNS = [
    ...
    (r"\b(too |very )?(long|lengthy) (sentences?|responses?|answers?|replies?)\b", 0.8),
    (r"\b(keep it |be )?(brief|short|concise|quick)\b",  0.7),
    (r"\bdon'?t (talk|write|speak|type) (so |too )?(long|much|verbose)\b", 0.8),
    (r"\bshorter (please|responses?|answers?)?\b",   0.7),
    (r"\bless (words?|text|talking)\b",              0.7),
]
